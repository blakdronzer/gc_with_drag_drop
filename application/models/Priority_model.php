...
class Priority_model extends CI_Model
{
    ...
    public function getRowsBefore($table, $primary_key='id', $priority_field, $group_field=FALSE, $sourceId, $distance) {
        if($group_field !== FALSE) {
            ...
            $sql = "select * from $table where `$priority_field` < (select `$priority_field` from $table where `$primary_key`='$sourceId') and `$group_field` = $group_val ORDER BY `$priority_field` DESC limit $distance";
        } else {
    ...
    public function getMinCount($table, $primary_key='id', $priority_field, $group_field=FALSE, $group_value=FALSE) {
        if($group_field !== FALSE && $group_value !== FALSE) {
            $this->db->where($group_field, $group_value);
        }
        $this->db->select_min($priority_field);
